    ...
    renderList() {
        const items = this.props.items.map(item => {
            if(item.time_text != null) {
                var distance = <littlebox2><h5>Walking time to location: {item.time_text}</h5></littlebox2>
            }
            return (
                <li key={item.name}>
                    <img alt={"photo of " + item.name} src={item.img}/>
                    <h3><i className="fa fa-home"></i> {item.name}</h3>
                    <littlebox><h4><i className="fa fa-map-marker"></i> <a href={item.brownMap} target="_blank"> Map</a></h4></littlebox>
                    {distance}
